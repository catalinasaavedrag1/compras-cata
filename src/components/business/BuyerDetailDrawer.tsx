...
import { leagueOf, scoreColor, WORKLOAD_CFG, trendText, trendColor } from "../../utils/teamScore";
import { formatCurrencyCompact, formatNumber, formatPercent } from "../../utils/formatters";
import { PILL_TONE as TONE_AV } from "../../utils/tone";
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden">
          <div
            ...
            style={{ width: `${goal.pct}%`, background: goalBar(goal.pct) }}
          />
        </div>
...
export function Sparkline({
  ...
  const points = data
    .map(
      (v, i) =>
        `${((i / (data.length - 1)) * width).toFixed(1)},${(height - ((v - min) / span) * height).toFixed(1)}`
    )
    .join(" ");
  return (
    <svg
...
export function BuyerDetailDrawer({
  ...
  const kpis: { label: string; value: string; tone?: "good" | "warn" | "bad" }[] = [
    {
      label: "Fill Rate",
      value: formatPercent(buyer.fillRate, 0),
      tone: buyer.fillRate >= 90 ? "good" : "warn",
    },
    {
      label: "Nivel de servicio",
      value: formatPercent(buyer.sla, 0),
      tone: buyer.sla >= 90 ? "good" : "warn",
    },
    {
      label: "Quiebres",
      value: String(buyer.stockouts),
      tone: buyer.stockouts > 6 ? "bad" : "warn",
    },
    {
      label: "Quiebres críticos",
      value: String(buyer.critical),
      tone: buyer.critical > 2 ? "bad" : "good",
    },
    ...
    {
      label: "T. reposición",
      value: `${buyer.replenDays} d`,
      tone: buyer.replenDays > 14 ? "bad" : undefined,
    },
    {
      label: "Rotación",
      value: `${buyer.rotation.toLocaleString("es-CL", { maximumFractionDigits: 1 })}x`,
    },
    ...
    {
      label: "Crecimiento",
      value: `${buyer.growth >= 0 ? "+" : ""}${formatPercent(buyer.growth, 1)}`,
      tone: buyer.growth >= 0 ? "good" : "bad",
    },
  ];
  const kpiColor = {
  ...
  return (
    <Drawer
      ...
      footer={
        <Button onClick={() => navigate("/equipo/carga")}>
          Equilibrar carga de este comprador
        </Button>
      }
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span
              className={`w-12 h-12 rounded-full flex items-center justify-center text-base font-semibold ${TONE_AV[buyer.tone]}`}
            >
              {buyer.initials}
            </span>
            <div>
              <p
                ...
                style={{ color: scoreColor(buyer.score) }}
              >
                {buyer.score}
              </p>
              <p className="text-xs font-semibold" style={{ color: scoreColor(buyer.score) }}>
                Nivel {leagueOf(buyer.score).league.name}
              </p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-1.5">
            <Badge tone={wl.tone}>
              Carga {wl.label} · {buyer.workloadPct}%
            </Badge>
            <span className="text-xs font-semibold" style={{ color: trendColor(buyer.trend) }}>
              {trendText(buyer.trend)} vs sem. anterior
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {kpis.map((k) => (
              <div key={k.label} className="rounded-lg border border-slate-200 px-3 py-2">
                <p
                  className={`text-base font-semibold ${k.tone ? kpiColor[k.tone] : "text-slate-800"}`}
                >
                  {k.value}
                </p>
            ...
            {buyer.breakdown.map((x) => (
              <div key={x.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div
                    className="h-full rounded-full"
                    style={{ width: `${x.value}%`, background: goalBar(x.value) }}
                  />
                </div>
                <span
                  className="w-8 text-right text-xs font-semibold"
                  style={{ color: goalBar(x.value) }}
                >
                  {x.value}
                </span>
