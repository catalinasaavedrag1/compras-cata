...
function Avatar({ id, size = "w-9 h-9" }: { id: string; size?: string }) {
  ...
  return (
    <span
      className={`${size} rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${BUYER_TONE_AV[b.tone]}`}
    >
      {b.initials}
    </span>
  );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5">
      {items.map((c) => {
        ...
        if (c.kind === "duel") {
          const aMe = c.aId === meId,
            bMe = c.bId === meId;
          const aWin = c.leaderId === c.aId;
          const a = getBuyer(c.aId),
            b = getBuyer(c.bId);
          return (
            <Card key={c.id}>
              <CardBody>
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`flex-1 flex items-center gap-2.5 rounded-lg px-3 py-2 ${aWin ? "bg-emerald-50" : "bg-slate-50"}`}
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p
                        ...
                        style={{ color: aWin ? "#059669" : "#64748b" }}
                      >
                        {c.aText}
                      </p>
                    </div>
                    {aWin && (
                      <span className="ml-auto text-[10px] font-bold text-emerald-600">LIDERA</span>
                    )}
                  </div>
            ...
            <CardBody>
              ...
              <div className="flex items-center gap-1.5">
                {Array.from({ length: c.goalWeeks }).map((_, i) => (
                  <span
                    key={i}
                    className={`flex-1 h-2 rounded-full ${i < c.weeks ? "bg-emerald-500" : "bg-slate-100"}`}
                  />
                ))}
              </div>
              <div className="flex items-center justify-between mt-1.5">
                <span className="text-xs font-semibold text-slate-700">
                  {c.weeks}/{c.goalWeeks} semanas
                </span>
                <span className="text-[11px] text-slate-400">{ends}</span>
              </div>
            </CardBody>
