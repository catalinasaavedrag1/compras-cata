  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-4 text-sm">
            <span className="text-slate-500">Promedio <b className="text-slate-800">{teamAvg}</b></span>
            <span className="text-slate-500"><b className="text-rose-600">{riskN}</b> en riesgo</span>
          </div>
        }
      />
              ...
              <div key={b.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium px-1.5 rounded" style={{ color }}>{lvl.label}</span>
                  ...
                  <div className="h-2 rounded-full bg-slate-100 overflow-hidden mt-1.5">
                    <div className="h-full rounded-full transition-all" style={{ width: `${okr}%`, background: color }} />
                  </div>
                </div>
                <span className="text-xl font-bold text-slate-900 w-12 text-right flex-shrink-0">{okr}</span>
              </div>
            );
          })}
        </CardBody>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <Select value={estado} onChange={(e) => setEstado(e.target.value)} options={[{ value: "all", label: "Todas las metas" }, { value: "risk", label: "Solo en riesgo" }, { value: "on_track", label: "Solo en curso" }, { value: "done", label: "Solo cumplidas" }]} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3.5">
        {cards.map(({ b, okr }) => {
          ...
          const topGoal = b.goals.filter((g) => g.status !== "done").sort((x, y) => goalImpact(y.weight, y.pct) - goalImpact(x.weight, x.pct))[0];
            ...
            <Card key={b.id}>
              <CardBody className="space-y-3">
                ...
                <div className="flex items-center gap-3.5">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: `conic-gradient(${color} ${okr * 3.6}deg, #eef2f7 0)` }}>
                    <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center">
                      <span className="text-lg font-bold" style={{ color }}>{okr}</span>
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      ...
                      {myRecog.map((r) => (
                        <span key={r} className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-700">🏅 {r}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                {topGoal && (
                  <p className="text-xs text-slate-600 bg-slate-50 rounded-lg px-3 py-2">
                    ⬆ Mayor impacto: <b className="text-slate-800">{topGoal.name}</b> <span className="text-emerald-700 font-semibold">+{goalImpact(topGoal.weight, topGoal.pct)} pts</span>
                  </p>
                )}
