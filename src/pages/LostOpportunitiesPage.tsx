...
export function LostOpportunitiesPage() {
  ...
  const filtered = all.filter((o) => {
    if (
      query.trim() &&
      !`${o.name} ${o.sku} ${o.category}`.toLowerCase().includes(query.toLowerCase())
    )
      return false;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
        ...
        <KpiCard
          ...
          value={`${formatCurrencyCompact(ventaPerdidaTotal)}/mes`}
          tone="bad"
          icon={<IconBulb className="w-4 h-4" />}
        />
        <KpiCard
          title="Por reponer urgente"
          value={formatNumber(all.filter((o) => o.tone === "red").length)}
          tone="bad"
          icon={<IconBulb className="w-4 h-4" />}
        />
      </div>
      ...
      {filtered.length === 0 ? (
        <Card>
          <CardBody>
            <EmptyState
              title="Sin oportunidades no capturadas"
              description="No hay productos que cumplan el patrón con los filtros actuales."
            />
          </CardBody>
        </Card>
      ) : (
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      · vendía ~{formatNumber(o.histMonthly)}/mes · ahora {formatNumber(o.recent)}
                      /mes
                    </p>
