...
export function MyPerformancePage() {
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: `${me.replenDays} d`,
      avg: `${agg.replen} d`,
      good: me.replenDays <= agg.replen,
    },
    {
      label: "Quiebres",
      value: String(me.stockouts),
      avg: String(Math.round(agg.stockouts / agg.n)),
      good: me.stockouts <= agg.stockouts / agg.n,
    },
  ];

  // Mejor posición en rankings específicos (encuadre positivo)
  const myRanks = RANKING_DEFS.map((def) => {
    const order = [...buyers].sort((a, b) =>
  ...
  return (
    <div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm font-medium text-slate-800 leading-snug">
              ...
              <span className="font-semibold text-brand-700 whitespace-nowrap">
                (+{improvements[0].pts} pts)
              </span>
            </p>
          </div>
          <span className="text-sm font-semibold text-brand-700 whitespace-nowrap group-hover:underline">
            Ir a reposición →
          </span>
        </Link>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-4 mb-4">
        ...
        <Card>
          <CardBody className="flex flex-col items-center justify-center text-center py-6">
            ...
            >
              <div className="w-24 h-24 rounded-full bg-white flex flex-col items-center justify-center">
                <span
                  ...
                  style={{ color: scoreColor(me.score) }}
                >
                  {me.score}
                </span>
            ...
            <Badge tone={league.tone} className="mt-3">
              Nivel {league.name}
            </Badge>
            <p className="text-xs font-semibold mt-1.5" style={{ color: trendColor(me.trend) }}>
              {trendText(me.trend)} vs semana anterior
            </p>
            {next && (
              <div className="w-full mt-3">
                ...
                <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.max(6, ((me.score - league.min) / (next.min - league.min)) * 100))}%`,
                      background: league.color,
                    }}
                  />
                </div>
                <p className="text-[11px] text-slate-500 mt-1.5">
                  Faltan <b className="text-slate-700">{ptsToNext} pts</b> para {next.name}
                </p>
              </div>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            <Card>
              <CardBody className="text-center py-4">
                ...
                <p className="text-3xl font-bold text-brand-700 mt-1">#{pos}</p>
                <p className="text-[11px] text-slate-400">de {agg.n} compradores</p>
              </CardBody>
            </Card>
            <Card>
              <CardBody className="text-center py-4">
                <p className="text-xs text-slate-500">Percentil</p>
                <p className="text-3xl font-bold text-slate-700 mt-1">{percentile}</p>
                <p className="text-[11px] text-slate-400">vs carga similar</p>
              </CardBody>
            </Card>
            <Card>
              <CardBody className="text-center py-4">
                <p className="text-xs text-slate-500">Cumpl. metas</p>
                <p className="text-3xl font-bold text-emerald-700 mt-1">{me.goalComp}%</p>
                <p className="text-[11px] text-slate-400">objetivos del mes</p>
              </CardBody>
            </Card>
          </div>
          {/* Encuadre positivo */}
              ...
              <p className="text-sm text-slate-700 leading-snug">
                ...
                {bestRank && bestRank.rank <= 3 && (
                  <>
                    {" "}
                    — y eres{" "}
                    <b style={{ color: scoreColor(90) }}>
                      {bestRank.rank === 1 ? "#1 del equipo" : `#${bestRank.rank}`}
                    </b>{" "}
                    en {RANK_PHRASE[bestRank.key]}.
                  </>
                )}
              </p>
            </CardBody>
          ...
          <Card>
            <CardBody>
              <p className="text-sm font-semibold text-slate-800 mb-3">
                Tus indicadores vs promedio del equipo
              </p>
              <div className="grid grid-cols-2 gap-2.5">
                {kpis.map((k) => (
                  <div
                    key={k.label}
                    className="flex items-center justify-between gap-2 border border-slate-100 rounded-lg px-3 py-2.5"
                  >
                    <span className="text-xs text-slate-500">{k.label}</span>
                    <span className="flex items-baseline gap-1.5">
                      <span
                        className={`text-sm font-semibold ${k.good ? "text-emerald-700" : "text-rose-700"}`}
                      >
                        {k.value}
                      </span>
      ...
      {/* Tu temporada */}
      <div
        className="rounded-2xl p-5 mb-4 text-white flex flex-wrap items-center gap-5"
      ...
      >
        <div className="flex-1 min-w-[200px]">
          ...
          <p className="text-xs text-indigo-200 mt-1">
            Estás en nivel <b className="text-white">{league.name}</b>.{" "}
            {next ? `Te faltan ${ptsToNext} pts para ${next.name}.` : "Estás en la cima."}
          </p>
        </div>
        <div className="bg-white/10 rounded-xl px-4 py-3 text-center">
          <p className="text-[11px] text-indigo-200">Si cerrara hoy</p>
          <p className="text-sm font-bold mt-0.5">
            {seasonCfg.arrow} {seasonCfg.label}
          </p>
          <p className="text-[11px] text-indigo-200 mt-0.5">
            {season.from.name} → {season.to.name}
          </p>
        </div>
      </div>
          ...
          <Card className="mb-4">
            <CardBody>
              ...
              <p className="text-xs text-slate-400 mb-3">
                No todo quiebre es culpa del comprador. Se separa la causa para una evaluación
                justa.
              </p>
              ...
              <p className="text-sm text-emerald-800 bg-emerald-50 rounded-lg px-3 py-2">
                ...
                {attr.scoreAdjust > 0 && (
                  <>
                    {" "}
                    <b>Estos quiebres no deberían penalizar tu score.</b>
                  </>
                )}
              </p>
            </CardBody>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-4 mb-4">
        <Card>
          <CardBody>
            ...
            <p className="text-xs text-slate-400 mb-3">
              Foco en tus dimensiones más débiles, ordenadas por impacto.
            </p>
            <div className="space-y-2.5">
              {improvements.map((imp) => (
                <div
                  key={imp.label}
                  className="flex items-start gap-3 border border-slate-100 rounded-lg px-3 py-2.5"
                >
                  <span className="inline-flex items-center justify-center rounded-md bg-emerald-50 text-emerald-700 text-xs font-bold px-2 py-1 flex-shrink-0">
                    +{imp.pts} pts
                  </span>
          ...
          <CardBody>
            ...
            {myBadges.length === 0 ? (
              <p className="text-sm text-slate-400">
                Aún no lideras ninguna insignia. Revisa "Cómo subir mi score" para acercarte.
              </p>
            ) : (
      ...
      <div className="mb-4">
        <ChallengeList items={myChallenges} meId={me.id} />
      </div>
          ...
          <CardBody>
            ...
            <div className="space-y-2">
              {rewardsInPlay.map(({ reward, winner, iWin }) => (
                <div
                  key={reward.id}
                  className={`rounded-lg border px-3 py-2 ${iWin ? "border-emerald-200 bg-emerald-50/60" : "border-slate-100"}`}
                >
                  ...
                  <p
                    className={`text-[11px] mt-1 ${iWin ? "text-emerald-700 font-semibold" : "text-slate-400"}`}
                  >
        ...
        <Card>
          <CardBody>
            ...
            <p className="text-[11px] text-slate-400 mb-3">
              Anonimizado — solo ves tu posición y el resto resumido.
            </p>
            <div className="space-y-1.5">
              {sorted.map((b, i) => {
                ...
                return (
                  <div
                    key={b.id}
                    className={`flex items-center gap-3 rounded-lg px-3 py-2 ${isMe ? "bg-brand-50" : ""}`}
                  >
                    <span className="w-6 text-center text-sm font-bold">
                      {i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}
                    </span>
                    <span className="flex-1 text-sm font-medium text-slate-700">
                      {isMe ? "Tú" : `Comprador ${String.fromCharCode(65 + i)}`}
                    </span>
                    <span className="text-sm font-bold" style={{ color: scoreColor(b.score) }}>
                      {b.score}
                    </span>
                  </div>
                );
