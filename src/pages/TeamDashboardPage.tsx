...
export function TeamDashboardPage() {
  ...
  const kpis: {
  ...
  }[] = [
    {
      ...
      value: `${a.goalArea}%`,
      sub: "Meta del área: 85%",
      tone: a.goalArea >= 85 ? "good" : "warn",
    },
    {
      title: "Fill Rate del área",
      value: `${a.fillRate}%`,
      sub: "Pedidos completos",
      tone: a.fillRate >= 90 ? "good" : "warn",
    },
    {
      title: "Nivel de servicio",
      value: `${a.sla}%`,
      sub: "Cumplimiento SLA",
      tone: a.sla >= 90 ? "good" : "warn",
    },
    {
      title: "Quiebres totales",
      value: formatNumber(a.stockouts),
      sub: `${a.critical} críticos`,
      tone: "bad",
    },
    {
      title: "Compras pendientes",
      value: formatNumber(a.pending),
      sub: `${a.openPO} OC abiertas`,
      tone: "warn",
    },
    {
    ...
    {
      title: "Sobrestock",
      value: formatNumber(a.overstock),
      sub: "SKUs sobre el máximo",
      tone: "neutral",
    },
    {
      title: "Productos gestionados",
      value: formatNumber(a.products),
      sub: `${a.categories} categorías · ${a.suppliers} prov.`,
      tone: "neutral",
    },
  ];
  ...
  const PerfRow = ({ b, sub }: { b: Buyer; sub: string }) => (
    <button
      onClick={() => setSel(b)}
      className="flex items-center gap-3 w-full text-left rounded-lg px-2 py-2.5 hover:bg-slate-50"
    >
      <span
        className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0 ${BUYER_TONE_AV[b.tone]}`}
      >
        {b.initials}
      </span>
      ...
      <span className="text-right">
        <span className="block text-lg font-bold" style={{ color: scoreColor(b.score) }}>
          {b.score}
        </span>
        <span className="block text-[11px] font-semibold" style={{ color: trendColor(b.trend) }}>
          {trendText(b.trend)}
        </span>
      </span>
    </button>
        ...
        >
          <div>
            ...
            <span className="inline-block mt-1 text-sm font-semibold text-emerald-300">
              {scoreLabel(a.avgScore)}
            </span>
          </div>
          <div className="flex gap-5 mt-5">
            <div>
              <p className="text-[11px] text-indigo-300">Compradores</p>
              <p className="text-base font-semibold">{a.n}</p>
            </div>
            <div>
              <p className="text-[11px] text-indigo-300">Venta del equipo</p>
              <p className="text-base font-semibold">{formatCurrencyCompact(a.sales)}</p>
            </div>
            <div>
              <p className="text-[11px] text-indigo-300">Ahorro negociado</p>
              <p className="text-base font-semibold">{formatCurrencyCompact(a.savings)}</p>
            </div>
          </div>
        </div>
        ...
        <Card>
          <CardHeader
            ...
            action={
              ...
              >
                Ver todas ({leaderAlerts.filter((x) => x.severity === "high").length})
              </button>
            }
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
        <Card>
          ...
          <CardBody className="py-2">
            {top.map((b) => (
              <PerfRow key={b.id} b={b} sub={b.categories.join(" · ")} />
            ))}
          </CardBody>
        </Card>
        <Card>
          ...
          <CardBody className="py-2">
            {bottom.map((b) => (
              <PerfRow
                key={b.id}
                b={b}
                sub={`${b.stockouts} quiebres · carga ${WORKLOAD_CFG[b.workload].label}`}
              />
            ))}
          </CardBody>
        </Card>
          ...
          {workload.map((b) => {
            ...
            return (
              <div key={b.id} className="flex items-center gap-3">
                ...
                <span className="w-32 flex-shrink-0 text-sm font-medium text-slate-700 truncate">
                  {b.name}
                </span>
                ...
                <span className="w-28 flex-shrink-0 text-right text-xs font-semibold text-slate-600">
                  {wl.label} {b.workloadPct}%
                </span>
              </div>
            );
          })}
